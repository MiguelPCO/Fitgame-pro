...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const profileLoadPromiseRef = useRef<{ uid: string; promise: Promise<boolean> } | null>(null);
  const profileRetryAfterRef = useRef<Map<string, number>>(new Map());

  const loadStoredUserSession = useCallback((requiresLocalSession = false): boolean => {
    ...
    if (!storedUser || (requiresLocalSession && !hasSession)) return false;
    ...
    if (validatedStreak !== storedUser.streak) {
  ...
  const loadUserData = useCallback(async (uid: string) => {
    ...
    const [supabaseTemplates, history, prs] = await Promise.all([
      fetchTemplates(uid),
  ...
  const loadUserProfile = useCallback(async (uid: string, force = false): Promise<boolean> => {
    if (!force && loadedProfileUserIdRef.current === uid) return true;

    const inFlight = profileLoadPromiseRef.current;
    if (!force && inFlight?.uid === uid) return inFlight.promise;

    const retryAfter = profileRetryAfterRef.current.get(uid) ?? 0;
    ...
    const loadPromise = (async () => {
      ...
      if (error) {
        ...
        profileRetryAfterRef.current.set(uid, Date.now() + 10000);
        return false;
      }
      ...
      profileRetryAfterRef.current.delete(uid);
      return true;
    })()
      .catch((err) => {
      ...
      .finally(() => {
        if (profileLoadPromiseRef.current?.uid === uid) {
          profileLoadPromiseRef.current = null;
        }
      });

    profileLoadPromiseRef.current = { uid, promise: loadPromise };
    return loadPromise;
  }, [loadUserData, toast]);
  ...
  useEffect(() => {
    ...
    if (isSupabaseConfigured()) {
      const { unsubscribe } = onAuthStateChange((authUser) => {
        void (async () => {
          if (authUser) {
            const profileLoaded = await loadUserProfile(authUser.id);
            if (profileLoaded) setIsAuthenticated(true);
          } else {
            loadedProfileUserIdRef.current = null;
            profileLoadPromiseRef.current = null;
            profileRetryAfterRef.current.clear();
            setUser(null);
            setUserId(null);
            setIsAuthenticated(false);
          }
        })().catch((err) => logger.error('Error handling auth change:', err));
      });

      return () => unsubscribe();
    }
  }, [loadStoredUserSession, loadUserProfile]);
